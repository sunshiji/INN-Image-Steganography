...
def gauss_noise(shape, device):
    noise = torch.zeros(shape).to(device)  # 将噪声张量创建在指定设备上
    for i in range(noise.shape[0]):
        noise[i] = torch.randn(noise[i].shape).to(device)  # 在指定设备上生成随机噪声
    ...
    for i_epoch in range(c.epochs):
        ...
        for i_batch, data in enumerate(datasets.trainloader):
            ...
            if i_batch == 1:
                ...
                print(f"cover_input:{cover_input.shape}")
                print(f"secret_input:{secret_input.shape}")
            ...
            output_z_guass = gauss_noise(output_z.shape, device)
        ...
        if i_epoch % c.val_freq == 0:
            with torch.no_grad():
                ...
                for x in datasets.testloader:
                    ...
                    output_z = output.narrow(1, 4 * c.channels_in, output.shape[1] - 4 * c.channels_in)
                    output_z = gauss_noise(output_z.shape, device)
