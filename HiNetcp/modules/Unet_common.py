...
# 获取Unet_common.py文件的绝对路径
current_file_path = os.path.abspath(__file__)

# 获取Unet_common.py文件所在的目录
current_dir = os.path.dirname(current_file_path)

# 获取Unet_common.py文件的父目录的父目录，即HiNet的根目录
root_dir = os.path.dirname(os.path.dirname(current_dir))

# 获取config.py文件的绝对路径
config_path = os.path.join(root_dir, 'config.py')

# 将config.py文件所在的目录添加到sys.path
...
class ResidualDenseBlock(nn.Module):
    def __init__(self, nf=64, gc=32, kernel_size = 3, bias=True, use_snorm=False):
        ...
        # 谱归一化（SpectralNormalization） 是一种正则化技术，用来约束卷积层的权重
        if use_snorm:
